    ...
    methods: {
        ...
        getUser: function () {
            $.getJSON(baseURL + "sys/user/info", function (r) {
                if (r.code == "500") {
                    location.href = "/armor/login.html";
                }
                vm.user = r.user;
                var userId = vm.user.username;

                tool.ajaxHelper.ajaxSubmit({userId: userId}, Api.ex_user.checkUserType, "GET", function (data) {
                    if (data.code = "0") {
                        window.roleList = data.data.roleList;
                        window.userInfo = data.data.userInfo;
                        vm.$data.user.username = userInfo.suName;
                    }
                })
