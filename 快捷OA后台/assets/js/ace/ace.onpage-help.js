jQuery(function($) {
	...
	var get_file_url = function(url, language) {
		//function that return the real path to a fileinput which is being loaded
		return this.settings.base + '/' + url;
	}
	var get_section_url = function(section_name) {
		//according to a section_name such as `basics/navbar.toggle` return the fileinput url which contains help content
		section_name = section_name || '';

		//for example convert `basic/navbar.layout.brand` to `basic/navbar`
		//because 'layout.brand' section is inside `basic/navbar.html` fileinput
	...
	var code_highlight = function(e, language) {
		...
		if(typeof e === 'string') {
			if(typeof language === 'string') {
				//called when a fileinput (html,css,etc) is loaded
