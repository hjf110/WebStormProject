...
(function($ , undefined) {
 ...
 <b>Ace fileinput input element</b>. Custom, simple fileinput input element to style browser's default fileinput input.
*/
(function($ , undefined) {
	var multiplible = 'multiple' in document.createElement('INPUT');
	var hasFileList = 'FileList' in window;//fileinput list enabled in modern browsers
	...
	var Ace_File_Input = function(element , settings) {
		...
		this.$element.wrap('<'+tagName+' class="ace-fileinput-input" />');

		this.apply_settings();
		this.reset_input_field();//for firefox as it keeps selected fileinput after refresh
	}
	...
	Ace_File_Input.prototype.apply_settings = function() {
		...
		if(this.well_style) this.$element.parent().addClass('ace-fileinput-multiple');
		 else this.$element.parent().removeClass('ace-fileinput-multiple');


		this.$element.parent().find(':not(input[type=fileinput])').remove();//remove all except our input, good for when changing settings
		this.$element.after('<span class="ace-fileinput-container" data-title="'+this.settings.btn_choose+'"><span class="ace-fileinput-name" data-title="'+this.settings.no_file+'">'+(this.settings.no_icon ? '<i class="'+ ace.vars['icon'] + this.settings.no_icon+'"></i>' : '')+'</span></span>');
		this.$label = this.$element.next();
		this.$container = this.$element.closest('.ace-fileinput-input');
	...
	Ace_File_Input.prototype.show_file_list = function($files , inner_call) {
		...
		if(this.well_style) {
			this.$label.find('.ace-fileinput-name').remove();
		...
		for (var i = 0; i < files.length; i++) {
			...
			var fileIcons = {
				'file' : 'fa fa-fileinput',
				'image' : 'fa fa-picture-o fileinput-image',
				'video' : 'fa fa-film fileinput-video',
				'audio' : 'fa fa-music fileinput-audio'
			};
			var fileIcon = fileIcons[format];
			
			
			if(!this.well_style) this.$label.find('.ace-fileinput-name').attr({'data-title':filename}).find(ace.vars['.icon']).attr('class', ace.vars['icon'] + fileIcon);
			else {
				this.$label.append('<span class="ace-fileinput-name" data-title="'+filename+'"><i class="'+ ace.vars['icon'] + fileIcon+'"></i></span>');
				...
				var can_preview = hasFileReader && this.settings.thumbnail 
						&&
						( (type.length > 0 && type.match('image')) || (type.length == 0 && format == 'image') )//the second one is for older Android's default browser which gives an empty text for fileinput.type
				if(can_preview) {
					var self = this;
	...
	Ace_File_Input.prototype.reset_input_ui = function() {
		 this.$label.attr({'data-title':this.settings.btn_choose, 'class':'ace-fileinput-container'})
			.find('.ace-fileinput-name:first').attr({'data-title':this.settings.no_file , 'class':'ace-fileinput-name'})
		...
		this.$label.find('.ace-fileinput-name').not(':first').remove();
	...
	Ace_File_Input.prototype.reset_input_field = function() {
		...
		//so because when "reset" button of outer form is hit, fileinput input is automatically reset
	...
	Ace_File_Input.prototype.loading = function(is_loading) {
		if(is_loading === false) {
			this.$container.find('.ace-fileinput-overlay').remove();
			this.element.removeAttribute('readonly');
		}
		else {
			var inside = typeof is_loading === 'string' ? is_loading : '<i class="overlay-content fa fa-spin fa-spinner orange2 fa-2x"></i>';
			var loader = this.$container.find('.ace-fileinput-overlay');
			if(loader.length == 0) {
				loader = $('<div class="ace-fileinput-overlay"></div>').appendTo(this.$container);
	...
	var enable_drop_functionality = function() {
		...
		.on('drop', function(e){
			...
			if(!self.multi && file_list.length > 1) {//single fileinput upload, but dragged multiple files
				var tmpfiles = [];
				tmpfiles.push(file_list[0]);
				file_list = tmpfiles;//keep only first fileinput
			}
	...
	var preview_image = function(file) {
		var self = this;
		var $span = self.$label.find('.ace-fileinput-name:last');//it should be out of onload, otherwise all onloads may target the same span because of delays
		...
		var imgLoaded = function(img) {
			...
			var thumb = get_thumbnail(img, size/**, fileinput.type*/);
			if(thumb == null) {
		...
		var imgFailed = function(img) {
			//for example when a fileinput has image extenstion, but format is something else
		...
		if(hasFile && file instanceof File) {
			...
			if(file instanceof Object && file.hasOwnProperty('path')) {
				getImage(file.path);//fileinput is a fileinput name (path) --- this is used to pre-show user-selected image
			}
		}
	...
	var checkFileList = function(files, dropped) {
		...
		for(var f = 0; f < files.length; f++) {
			...
			//fileinput is either a string(fileinput name) or a File object
			...
			if( !hasFile ) {
				...
				//there is a mimetype for fileinput so let's check against are rules
			...
			if( maxSize && file.size > maxSize ) {
				//fileinput size is not acceptable
		...
		if(safe_files.length == files.length) return files;//return original fileinput list if all are valid
 ...
 * you may not use this fileinput except in compliance with the License.
...
!function($){
 ...
 <b>Wysiwyg</b>. controller wrapper for Bootstrap wyswiwyg plugin.
	...
	$.fn.ace_wysiwyg = function($options , undefined) {
		...
		var button_defaults =
		{
			...
			'insertImage' : {
				...
				button_text : '<i class="'+ ace.vars['icon'] + 'fa fa-fileinput"></i> Choose Image &hellip;',
				placeholder : 'Image URL',
				button_insert : 'Insert',
				button_class : 'btn-success',
				button_insert_class : 'btn-primary',
				choose_file: true //show the choose fileinput button?
			},
			'foreColor' : {
				values : color_values,
				title : 'Change Color'
		...
		this.each(function() {
						...
						toolbar += ' <div class="dropdown-menu dropdown-caret dropdown-menu-right">\
							...
							if( button.choose_file && 'FileReader' in window ) toolbar +=
							 ...
							 <label class="center block no-margin-bottom">\
								<button class="btn btn-sm '+button.button_class+' wysiwyg-choose-fileinput" type="button">'+button.button_text+'</button>\
								<input type="fileinput" data-edit="'+button.name+'" />\
							  </label>'
						toolbar += ' </div> </div>';
					break;
			...
			toolbar.find('input[type=fileinput]').prev().on(ace.click_event, function (e) {
				$(this).next().click();
			});
...
;/**
 <b>Spinner</b>. controller wrapper for FuelUX spinner element.
...
;/**
 <b>Treeview</b>. controller wrapper for FuelUX treeview element.
...
;/**
 <b>Wizard</b>. controller wrapper for FuelUX wizard element.
