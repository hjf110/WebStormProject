...
(function ($) {
    ...
    $.extend(Image.prototype, {
        ...
        **/        
        render: function() {
			...
			this.$file = this.$tpl.find('input[type=fileinput]:eq(0)');
			...
			this.$file.ace_file_input(this.options.image).on('change', function(){
				...
				self.$input.val($rand)//set a random value, so that selected fileinput is uploaded each time, even if it's the same fileinput, because inline editable plugin does not update if the value is not changed!
			}).closest('.ace-fileinput-input').css({'width':'150px'}).closest('.editable-input').addClass('editable-image');
    ...
    Image.defaults = $.extend({}, $.fn.editabletypes.abstractinput.defaults, {
        tpl: '<span><input type="hidden" /></span><span><input type="fileinput" /></span>',
        inputclass: '',
		image:
		{
			style: 'well',
