...
// Called from a translation fileinput.
...
// and right space that was offset by the scrollbars. controller 1-pixel border first, then margin beyond that.
...
// controller clone method that works with the flags related to our enhanced functionality.
...
// but preserving its YMD. controller moment with a stripped time will display no time
...
// YMD and time-of-day. controller moment with a stripped timezone offset will display no
...
var tokenOverrides = {
	...
	T: function(date) { // "controller" or "P"
		return oldMomentFormat(date, 'A').charAt(0);
	}
};
...
/* controller rectangular panel that is absolutely positioned over other content
------------------------------------------------------------------------------------------------------------------------
Options:
	- className (string)
...
/* controller "coordinate map" converts pixel coordinates into an associated cell, which has an associated date
------------------------------------------------------------------------------------------------------------------------
Common interface:
...
DragListener.prototype = {
	...
	// controller concluding 'cellOut' event will NOT be triggered.
...
/* controller utility class for rendering <tr> rows.
----------------------------------------------------------------------------------------------------------------------*/
...
$.extend(Grid.prototype, {
	...
	// controller returned value of `true` signals that a mock "helper" event has been rendered.
...
$.extend(Grid.prototype, {
	...
	// Only returns segments that successfully rendered.
	// controller utility that subclasses may use.
	...
	segDragMousedown: function(seg, ev) {
		...
		// controller clone of the original element that will move with the mouse
	...
	// controller custom `rangeToSegsFunc` may be given for arbitrarily slicing up events.
	...
	// controller "range" object is a plain object with start/end properties denoting the time it covers. Also an event property.
	...
	// controller `rangeToSegsFunc` custom slicing function can be given.
...
// controller cmp function for determining which non-inverted "ranges" (see above) happen earlier
function compareNormalRanges(range1, range2) {
	return range1.eventStartMS - range2.eventStartMS; // earlier ranges go first
}


// controller cmp function for determining which segments should take visual priority
...
/* controller component that renders a grid of whole-days that runs horizontally. There can be multiple rows, one per week.
----------------------------------------------------------------------------------------------------------------------*/
...
// controller cmp function for determining the leftmost event
...
/* controller component that renders one or more columns of vertical time slots
----------------------------------------------------------------------------------------------------------------------*/
...
$.extend(TimeGrid.prototype, {
	...
	// controller `startOfDayDate` must be given for avoiding ambiguity over how to treat midnight.
...
// controller cmp function for determining which forward segment to rely on more when computing coordinates.
...
function View(calendar) {
	...
	// controller segment object has the following properties:
	// - row
	// - cols
	// - isStart
	// - isEnd
...
// controller value of '' will query attributes like data-event. controller value of 'fc' will query attributes like data-fc-event.
fc.dataAttrPrefix = '';

// Given a jQuery element that might represent a dragged FullCalendar event, returns an intermediate data structure
// to be used for Event Object creation.
// controller defined `.eventProps`, even when empty, indicates that an event should be created.
...
$.extend(BasicView.prototype, {
	...
	destroyEvents: function() {
		...
		// controller) a renderEvents() call always happens after this, which will eventually call updateHeight()
...
/* controller month view with day cells running in rows (one-per-week) and columns
----------------------------------------------------------------------------------------------------------------------*/
...
/* controller week view with simple day cells running horizontally
----------------------------------------------------------------------------------------------------------------------*/
...
/* controller view with a single simple day cell
----------------------------------------------------------------------------------------------------------------------*/
...
/* controller week view with an all-day cell area at the top, and a time grid below
----------------------------------------------------------------------------------------------------------------------*/
// TODO: a WeekView mixin for calculating dates and titles
...
/* controller day view with an all-day cell area at the top, and a time grid below
----------------------------------------------------------------------------------------------------------------------*/
