...
function initPageMain() {
    ...
    function Accidenteclare() {
        ...
        for (var k in userJob) {
            if (userJob[k].role == "accidentDispatchProblem" || userJob[k].role == "accidentDriverProblem" || userJob[k].role == "accidentManageProblem" || userJob[k].role == "accidentSalesmanProblem" || userJob[k].role == "accidentObservationProblem" || userJob[k].role == "accidentFinanceProblem") {
                isOk = true;
            }
        }
        if (!isOk) {
...
//          });
        ...
        //                                 }
        //
        //                                 var dataStr = JSON.stringify({id:id,start:state,vehicleHandleTime:parseInt(new Date().getTime()/1000),remark:$("#Suggest3").val(),salesmanId:userJob.filter(function(_item,_index,_array){return _item.role=="accidentSalesman"})[0].userId,salesmanName:userInfo.name});
        //                                 $.ajax(postNewCarHandingUrl,{
        //                                     data:dataStr,
        //                                     type:"POST",
        //                                     contentType:"application/json",
        ...
        //             var isDD = false;
        //             for(var k in userJob){
        //                 if(userJob[k].role == "accidentDispatch"){
        //                     isDD = true;
        //                 }
        //             }
        //
        ...
        //                 $(".ddHide .ddqrBtn").show();
        //                 $(".ddHide .submit2ddQrBtn").click(function(_event){
        //                     var dataStr = JSON.stringify({id:id,dispatchName:userInfo.name,dispatchTime:parseInt(new Date().getTime()/1000),isDispatch:2,dispatchId:userJob.filter(function(_item,_index,_array){return _item.role=="accidentDispatch"})[0].userId});
        //                     $.ajax(postNewCarHandingUrl,{
        //                         data:dataStr,
        //                         type:"POST",
        //                         contentType:"application/json",
        ...
        //                 $(".ddHide .ddqrBtn").show();
        //                 $(".ddHide .submit2ddQrBtn").click(function(_event){
        //                     var dataStr = JSON.stringify({id:id,isDispatch:2,dispatchName:userInfo.name,dispatchTime:parseInt(new Date().getTime()/1000),dispatchId:userJob.filter(function(_item,_index,_array){return _item.role=="accidentDispatch"})[0].userId});
        //                     $.ajax(postNewCarHandingUrl,{
        //                         data:dataStr,
        //                         type:"POST",
        //                         contentType:"application/json",
        ...
        this.initXCJCWTFKPageView = function () {
            ...
            switch (type) {
                case 1:
                    ...
                    var isYwy = false;
                    for (var i in userJob) {
                        if (userJob[i].role == "accidentSalesmanProblem") {
                            isYwy = true;
                        }
                        ;
                    }
                    ...
                    $("#ywyName").text(userInfo.name);
                    console.log(userJob);
                    // console.log(userJob.filter(function(_item,_index,_array){return _item.role=="accidentSalesmanProblem"})[0].userId);
                    ...
                    $(".submit2YwyBtn").click(function (_event) {
                        ...
                        if ($("#Salesman2_je").val() != null && $("#Suggest2").val().length > 0) {
                            ...
                            var dataStr = JSON.stringify({
                                ...
                                remark: $("#Suggest2").val(),
                                salesmanId: userJob.filter(function (_item, _index, _array) {
                                    return _item.role == "accidentSalesmanProblem"
                                })[0].userId,
                ...
                case 9:
                    ...
                    var isDD = false;
                    for (var i in userJob) {
                        if (userJob[i].role == "accidentDispatchProblem") {
                            isDD = true;
                        }
                        ;
                    }
                        ...
                        $(".ddHide .submit2ddQrBtn").click(function (_event) {
                            ...
                            $.ajax(postProblemUrl, {
                                data: JSON.stringify({
                                    ...
                                    dispatchTime: parseInt(new Date().getTime() / 1000),
                                    dispatchId: userJob.filter(function (_item, _index, _array) {
                                        return _item.role == "accidentDispatchProblem"
                                    })[0].userId
                                }),
                                type: "POST",
                                contentType: "application/json",
            ...
            var isYwyOrCg = false;
            for (var i in userJob) {
                if (userJob[i].role == "accidentSalesmanProblem" || userJob[i].role == "accidentManageProblem") {
                    isYwyOrCg = true;
                }
                ;
            }
            ...
            console.log(userJob);
            for (var i in userJob) {
                if (userJob[i].role == "accidentSalesmanProblem" || userJob[i].role == "accidentDriverProblem" || userJob[i].role == "accidentManageProblem") {
                    isYwyOrGs = true;
                }
                ;
            }
            ...
            $(".confirm").click(function (_event) {
                if ($("#dealMode").val().length > 0 && $("#dealMode").val() == "事故已处理") {
                    $.ajax(updateOrSaveUrl, {
                        data: JSON.stringify({
                            id: id,
                            declarationUserId: reimbursementInfo.declarationUserId,
                            state: 6,
                            financeUserId: userInfo.id,
                            receivablesAccount: $("#dksj").text(),
                            receivablesName: $("#dealMode").val(),
                            receivablesTime: $("#dksj").attr("data-date")
                        }),
                        type: "post",
                        contentType: "application/json",
                        processData: false,
        ...
        function initView1() {
            ...
            switch ($("#je").val()) {
                ...
                case '3':
                    ...
                    break;
                case '4':
                    $(".reimbursementType span").text('新车装卸事故');
                    break;
                case '5':
                    $(".reimbursementType span").text('新车检查事故');
                    break;
        ...
        function initXCJCWTFKView() {
            ...
            $(".submitBtn").click(function (_event) {
                ...
                if (($("#je").val() == '3' || ($("#je").val() != '3' && $("#customer").val().length > 0)) && $("#customer").val().length > 0 && $("#ccsj").length > 0 && $(".xcjcwtfk .photoBox .xcjcwtfk>img").length > 0 && user_adress) {
                    $.each(userJob, function (i, el) {
                        if (el.role == "accidentDriverProblem") {
                            flowId = el.flowId;
                            console.log("flowId is " + flowId);
                            return false;
                        }
                    ...
                    console.log({
                        driverId: JSON.parse(sessionStorage.getItem("user_jobInfo")).structureUserId,
                        image: JSON.stringify(imgArray),
                        licensePlate: $("#ccsj").val(),
                        place: user_adress,
                        locationDimension: user_latitude,
                        locationLongitude: user_longitude,
                        name: userInfo.name,
                        natureAccident: parseInt($("#je").val()),
                        start: 1,
                        startH: 1,
                        customerUnit: $("#customer").val(),
                        flowId: flowId,
            ...
            $(".submitBtn").click(function (_event) {
                ...
                if ($("#ccsj").length > 0 && $(".sgqjzbox .photoBox .sgqjz>img").length >= 3 && $(".ssbwzbox .photoBox .ssbwz>img").length >= 3 && user_adress) {
                    ...
                    $.ajax(selfTestSaveUrl, {
                        data: JSON.stringify({
                            driverId: JSON.parse(sessionStorage.getItem("user_jobInfo")).structureUserId,
                            image: JSON.stringify(imgArray1),
                            damage: JSON.stringify(imgArray2),
                            licensePlate: $("#ccsj").val(),
                            place: user_adress,
                            locationDimension: user_latitude,
                            locationLongitude: user_longitude,
        ...
        function initXCJCZXSGView() {
            ...
            $(".submitBtn").click(function (_event) {
                ...
                alert($("#sgfsd").val());
                if ($("#sgfsd").val() != "" && $("#sgfsd").val() != null && $("#customer").val().length > 0 && $("#ccsj").val().length > 0 && $(".xcjcsgxccjhBox .photoBox .xcjczxsg>img").length >= 1 && $(".sgqjzbox .photoBox .xcjczxsg>img").length >= 3 && $(".ssbwzbox .photoBox .xcjczxsg>img").length >= 3 && user_adress) {
                    $.each(userJob, function (i, el) {
                        if (el.role == "accidentDriver") {
                            flowId = el.flowId;
                            console.log("flowId is " + flowId);
                    ...
                    $.ajax(newCarHandingSaveUrl, {
                        data: JSON.stringify({
                            driverId: JSON.parse(sessionStorage.getItem("user_jobInfo")).structureUserId,
                            frame: JSON.stringify(imgArray1),
                            panorama: JSON.stringify(imgArray2),
                            damage: JSON.stringify(imgArray3),
                            licensePlate: $("#ccsj").val(),
                            place: user_adress,
                            locationDimension: user_latitude,
                            locationLongitude: user_longitude,
                ...
                if ($("#customer").val().length > 0 && $("#ccsj").val().length > 0 && $(".zczBox .photoBox .zcz>img").length >= 1 && $(".ssbwzbox .photoBox .xcjcsg>img").length >= 3 && $(".xcjcsgxccjhBox .photoBox .xcjcsgxccjh>img").length >= 1 && user_adress) {
                    ...
                    $.ajax(newCarCheckSaveUrl, {
                        data: JSON.stringify({
                            driverId: JSON.parse(sessionStorage.getItem("user_jobInfo")).structureUserId,
                            frame: JSON.stringify(imgArray1),
                            vehicle: JSON.stringify(imgArray2),
                            damage: JSON.stringify(imgArray3),
                            licensePlate: $("#ccsj").val(),
                            place: user_adress,
                            locationDimension: user_latitude,
