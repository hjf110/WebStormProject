...
/*            alert(_data.name);*/
        ...
        for(var k in userJob){
            if(userJob[k].role == "accidentDispatchSelfTest"||userJob[k].role == "accidentDriverSelfTest"||userJob[k].role=="accidentManageSelfTest"||userJob[k].role=="accidentSalesmanSelfTest"||userJob[k].role=="accidentObservationSelfTest"||userJob[k].role=="accidentFinanceSelfTest"){
                isOk = true;
            }
        }
        if(!isOk){
            ...
            this.initZCJTSGPageView = function(){
                ...
                switch(type){
                    case 1:
                        ...
                        var isCg = false;
                        for(var j in userJob){
                            if(userJob[j].role == "accidentManageSelfTest"){
                                isCg = true;
                            };
                        }
                        ...
                        $(".submitCgClBtn").click(function(_event){
                            ...
                            if(($("#cg_je").val()=="2"&&$("#cgclyj").val().length>0)||$("#cg_je").val()=="1"){
                                ...
                                var dataStr = JSON.stringify({id:id,start:state,vehicleHandleTime:parseInt(new Date().getTime()/1000),remark:$("#cgclyj").val(),vehicleCompleteId:userJob.filter(function(_item,_index,_array){return _item.role=="accidentManageSelfTest"})[0].userId,vehicleCompleteName:userInfo.name});
                    ...
                    case 2:
                        ...
                        var isDD = false;
                        for(var i in userJob){
                            if(userJob[i].role == "accidentDispatchSelfTest"){
                                isDD = true;
                            };
                        }
                        if(isDD&&reimbursementInfo.dispatchName==null){
                            ...
                            $(".ddHide .submit2ddQrBtn").click(function(_event){
                                ...
                                dd.device.notification.showPreloader({
                                    ...
                                    $.ajax(postSelfTestUrl,{
                                        data:JSON.stringify({id:id,isDispatch:2,dispatchTime:parseInt(new Date().getTime()/1000),dispatchName:userInfo.name,dispatchId:userJob.filter(function(_item,_index,_array){return _item.role == "accidentDispatchSelfTest"})[0].userId}),
                                        type:"POST",
                                        contentType:"application/json",
                                        processData:false,
                                        success:function(_data) {
                    ...
                    case 5:
                        ...
                        var isSG = false;
                        for(var i in userJob){
                            if(userJob[i].role == "accidentDriverSelfTest"||userJob[i].role == "accidentObservationSelfTest"){
                                isSG = true;
                            };
                        }
                ...
                var isYwyOrCg = false;
                for(var i in userJob){
                    if(userJob[i].role == "accidentSalesmanSelfTest"||userJob[i].role == "accidentManageSelfTest"){
                        isYwyOrCg = true;
                    };
                }
                ...
                console.log(userJob);
                for(var i in userJob){
                    if(userJob[i].role == "accidentManageSelfTest"||userJob[i].role == "accidentDriverSelfTest"){
                        isYwyOrGs = true;
                    };
                }
                console.log(isYwyOrGs);
        ...
        function initZCJTSGView(){
            ...
            $(".submitBtn").click(function(_event){
                ...
                if($("#ccsj").length>0&&$(".sgqjzbox .photoBox .sgqjz>img").length>=3&&$(".ssbwzbox .photoBox .ssbwz>img").length>=3&&user_adress){
                    $.each(userJob, function (i, el) {
                        if (el.role == "accidentDriverSelfTest") {
                            flowId = el.flowId;
                            console.log("flowId is "+flowId);
                            return false;
                        }
