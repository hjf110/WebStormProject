...
/*            alert(_data.name);*/
        ...
        for(var k in userJob){
            if(userJob[k].role == "accidentDispatchHoldCar"||userJob[k].role == "accidentDriverHoldCar"||userJob[k].role=="accidentManageHoldCar"||userJob[k].role=="accidentSalesmanHoldCar"||userJob[k].role=="accidentObservationHoldCar"||userJob[k].role=="accidentFinanceHoldCar"){
                isOk = true;
            }
        }
        if(!isOk){
...
//          });
            ...
            this.initJCZXSGPageView = function(){
                ...
                switch(type){
                    case 1:
                        ...
                        for(var j in userJob){
                            if(userJob[j].role == "accidentManageHoldCar"){
                                isCg = true;
                            };
                        }
                        ...
                        $(".submitCgClBtn").click(function(_event){
                            ...
                            if(($("#cg_je").val()=="2"&&$("#cgclyj").val().length>0)||$("#cg_je").val()=="1"){
                                ...
                                var dataStr = JSON.stringify({id:id,start:state,vehicleHandleTime:parseInt(new Date().getTime()/1000),remark:$("#cgclyj").val(),vehicleCompleteId:userJob.filter(function(_item,_index,_array){return _item.role=="accidentManageHoldCar"})[0].userId,vehicleCompleteName:userInfo.name});
                                $.ajax(postOldCarUrl,{
                                    data:dataStr,
                                    type:"POST",
                                    contentType:"application/json",
                    ...
                    case 2:
                        ...
                        for(var i in userJob){
                            if(userJob[i].role == "accidentDispatchHoldCar"){
                                isDD = true;
                            };
                        }
                        ...
                        if(isDD&&reimbursementInfo.dispatchName==null){
                            ...
                            $(".ddHide .submit2ddQrBtn").click(function(_event){
                                ...
                                dd.device.notification.showPreloader({
                                    ...
                                    $.ajax(postOldCarUrl,{
                                        data:JSON.stringify({id:id,isDispatch:2,dispatchName:userInfo.name,dispatchTime:parseInt(new Date().getTime()/1000),dispatchId:userJob.filter(function(_item,_index,_array){return _item.role == "accidentDispatchHoldCar"})[0].userId}),
                                        type:"POST",
                                        contentType:"application/json",
                                        processData:false,
                                        success:function(_data) {
                    ...
                    case 5:
                        ...
                        var isSG = false;
                        for(var i in userJob){
                            if(userJob[i].role == "accidentDriverHoldCar"||userJob[i].role == "accidentObservationHoldCar"){
                                isSG = true;
                            };
                        }
                ...
                var isYwyOrCg = false;
                for(var i in userJob){
                    if(userJob[i].role == "accidentSalesmanHoldCar"||userJob[i].role == "accidentManageHoldCar"){
                        isYwyOrCg = true;
                    };
                }
                ...
                console.log(userJob);
                for(var i in userJob){
                    if(userJob[i].role == "accidentManageHoldCar"||userJob[i].role == "accidentDriverHoldCar"){
                        isYwyOrGs = true;
                    };
                }
    ...
    //        初始化新车检查问题反馈事故
        function initXCJCWTFKView(){
            ...
            $(".submitBtn").click(function(_event){
                ...
                if(($("#je").val()=='3'||($("#je").val()!='3'&&$("#customer").val().length>0))&&$("#customer").val().length>0&&$("#ccsj").length>0&&$(".xcjcwtfk .photoBox .xcjcwtfk>img").length>0&&user_adress){
                    $.each(userJob, function (i, el) {
                        if (el.role == "accidentDriverHoldCarHoldCar") {
                            flowId = el.flowId;
                            console.log("flowId is "+flowId);
                            return false;
                        }
        ...
        function initJSZXSGView(){
            ...
            $(".submitBtn").click(function(_event){
                ...
                //   if((!newCar&&isFall&&$(".invoice .photoBox>*[class=photo1]").length>0&&user_adress)||((newCar||$("#je").val()=='1')&&isFall&&$("#customer").val().length>=0&&$(".invoice .photoBox>*[class=photo1]").length>0&&user_adress))
                console.log($("#customer").val().length+"-----"+$("#ccsj").val().length+"-----"+$(".sgqjzbox .photoBox .photo1>img").length+"-----"+$(".ssbwzbox .photoBox .photo1>img").length+"----"+$("#ccsj").val().length+"----"+user_adress);
                if($("#customer").val().length>0&&$("#ccsj").val().length>0&&$(".sgqjzbox .photoBox .sgqjz>img").length>=3&&$(".ssbwzbox .photoBox .ssbwz>img").length>=3&&user_adress){
                    $.each(userJob, function (i, el) {
                        if (el.role == "accidentDriverHoldCar") {
                            flowId = el.flowId;
                            console.log("flowId is "+flowId);
                            return false;
                        }
